...
export default function Navbar() {
  ...
  const cta = navData?.cta || { text: "Get Started", link: "/contact#guaranteed-response" };
    ...
    <header className="sticky top-0 z-50 py-4 px-6 md:px-12 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
      <Link to="/" className="flex items-center gap-1 sm:gap-3">
        <img
          ...
          alt="Betwo Tech logo"
          className="w-12 h-12 sm:w-16 md:w-24 md:h-24 object-contain rounded-lg"
        />
        <span className="brand-name text-sm sm:text-lg md:text-2xl font-bold text-slate-900 dark:text-blue-400">
          {brand.name}
        </span>
      </Link>
      ...
      <div className="relative flex items-center gap-2 md:gap-4">
        ...
        <a href={cta.link} className="btn-primary hidden md:flex items-center justify-center px-5 py-2 rounded-full bg-slate-900 hover:bg-slate-800 dark:bg-sky-600 dark:hover:bg-sky-500 text-white font-extrabold transition-all ml-2">
          {cta.text}
        </a>
      </div>
        ...
        <div className="absolute top-full left-0 w-full bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 md:hidden">
          <nav className="flex flex-col p-6 gap-4 font-semibold dark:font-bold text-slate-700 dark:text-slate-100">
            ...
            <a 
              href={cta.link} 
              onClick={() => setIsOpen(false)}
              className="btn-primary mt-2 text-center px-5 py-3 rounded-full bg-slate-900 hover:bg-slate-800 dark:bg-sky-600 dark:hover:bg-sky-500 text-white font-extrabold transition-all"
            >
              {cta.text}
            </a>
          </nav>
